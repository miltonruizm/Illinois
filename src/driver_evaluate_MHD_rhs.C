...
static inline void reconstruct_set_of_prims(
    ...
    CCTK_REAL *restrict temporary)
{
    ...
    if (CCTK_EQUALS(reconstruction_method,"PPM")) {

        reconstruct_set_of_prims_PPM(
            eos,cctkGH,cctk_lsh,flux_dirn,
	...
	if(!printed) {
	  CCTK_INFO("IllinoisGRMHD: Using MP5 reconstruction");
	  printed = true;
	}
	...
	if(!printed) {
	  ...
	  printed = true;
        ...
        CCTK_ERROR("Unknown reconstruction_method");
...
if (dbg0 < 20) {
  ...
  printf("After curvature: proc=%d tau_rhs=% .16e\n",
         CCTK_MyProc(cctkGH), tau_rhs[idx]);
  dbg0++;
}
/*/
  ...
  // This function is housed in the file: "reconstruct_set_of_prims_PPM.C"
   ...
   reconstruct_set_of_prims(eos,cctkGH,cctk_lsh,flux_dirn,num_prims_to_reconstruct,which_prims_to_reconstruct,
...
if (dbg_x < 20) {
  ...
  printf("After X flux: proc=%d tau_rhs=% .16e\n",
         CCTK_MyProc(cctkGH), tau_rhs[idx]);
  dbg_x++;
}
...
/*/--
	...
	#pragma omp critical
        	{
            	printf("AFTER add_fluxes: i=%d j=%d k=%d tau_rhs=% .16e\n",
                	   i,j,k,tau_rhs[index]);
            	ndbg++;
       		 }
    		}
...
if (dbg_y < 20) {
    ...
    printf("After Y flux: proc=%d tau_rhs=% .16e\n",
           CCTK_MyProc(cctkGH), tau_rhs[idx]);
...
if (dbg_z < 20) {
    ...
    printf("After Z flux: proc=%d tau_rhs=% .16e\n",
           CCTK_MyProc(cctkGH), tau_rhs[idx]);
...
if (rhsdbg < 20) {
    ...
    printf("CENTER proc=%d i=%d j=%d k=%d tau=% .16e\n",
           CCTK_MyProc(cctkGH),
           i,j,k,
           tau_rhs[index]);
...
//#include "reconstruct_set_of_prims_MP5.C"
