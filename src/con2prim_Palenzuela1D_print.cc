...
inline int check_depsdT_condition( const igm_eos_parameters eos,
  ...
  WVU_EOS_get_eps_range(rho, ye, &eps_min, &eps_max);
  ...
  /*static int nprint = 0;
  if (nprint < 500 &&
      rho > 1.0e-8 && rho < 1.0e-7) {

    printf("BOUND EPS_CLAMP"
	   "rho=% .8e "
	   "tau=% .8e "
	   //"eps_energy=% .8e "
	   "eps_before=% .8e "
  ...
  //  WVU_EOS_P_S_T_and_depsdT_from_rho_Ye_eps( rho,ye,eps, &press,&ent,&temp,&depsdT );  //---> bug
...
if (rho > 1.e-10 &&
    rho < 1.e-6 &&
    depsdT < 100.0*eos.depsdT_threshold &&
    nprint < 500) {
...
void palenzuela( const igm_eos_parameters eos,
  ...
  if( stats.which_routine == None ) {
    ...
    if( (xlow_entropy_key == Palenzuela1D_entropy) ||
      ...
      if(entprint < 100) {printf("USING ENTROPY RECOVERY\n"); entprint++;}
      /*/
...
/*/--
 static int n1 = 0;
 if (con[DD] > 1.0e-8 && con[DD] < 1.0e-7 && n1 < 500) {
   printf("C2P_IN  rho_cons=% .8e tau_cons=% .8e\n",con[DD], con[TAU]);
   n1++;}
 /*/
  ...
  if( eos.evolve_T ) {
    if( stats.which_routine == Palenzuela1D ) {
      ...
      printf("ROOT367 x=% .16e W=% .16e rho=% .16e\n",
       x, W, rho);
...
/*/---
static int n2 = 0;
if (prim[RHO] > 1.0e-8 && prim[RHO] < 1.0e-7 && n2 < 500) {
  printf("C2P_OUT rho=% .8e tau_cons=% .8e eps=% .8e P=% .8e T=% .8e\n",
         prim[RHO],
         con[TAU],
         prim[EPS],
         prim[PRESS],
...
double func_root( const igm_eos_parameters eos,
  ...
  if( eos.evolve_T ) {
    if( stats.which_routine == Palenzuela1D ) {
      ...
      printf("ROOT501 x=% .16e W=% .16e rho=% .16e\n",
       x, W, rho);
 
      eps = W - 1.0 + (1.0-W*W)*x/W + W*(q - s + t*t/(2*x*x) + s/(2*W*W)  );

      //static int ncheck2 = 0;
...
double zbrent( double (*func)(const igm_eos_parameters, double, double *, double *restrict, output_stats&),
  ...
  printf("BRENT START x1=% .16e x2=% .16e\n", x1, x2);
...
printf("BRENT ITER iter=%d b=% .16e fb=% .16e\n",
       iter, b, fb);
  ...
  if( (!robust_isfinite(fb)) ) {
     printf("BRENT END FAILED iter=%d b=% .16e fb=% .16e\n",
           iter, b, fb);
    stats.c2p_failed = true;
    return b;
  } if( (fabs(maxerror) <= tol1 || fb == 0.0)){
     printf("BRENT END SUCCESS iter=%d ans=% .16e fb=% .16e\n",
           iter, ans, fb);
    stats.c2p_failed = false;
    return ans;
...
printf("BRENT END MAXITER iter=%d b=% .16e fb=% .16e\n",
           iter, b, fb);
    stats.c2p_failed = true;
    return b;
  }
