...
#pragma omp parallel for
  for(int k=cctk_nghostzones[2];k<cctk_lsh[2]-(cctk_nghostzones[2]-1);k++) for(int j=cctk_nghostzones[1];j<cctk_lsh[1]-(cctk_nghostzones[1]-1);j++) for(int i=cctk_nghostzones[0];i<cctk_lsh[0]-(cctk_nghostzones[0]-1);i++) {
	...
	if (fabs(tau_rhs_term) > 1.0e-6 && nsrc < 500) {
	#pragma omp critical
		{
    		printf("SRC i=%d j=%d k=%d term=% .16e\n",
           	i,j,k,tau_rhs_term);
    		nsrc++;
		}
	}
	...
	CCTK_REAL fluxterm =
   	 (tau_flux[index]-tau_flux[indexp1])*dxi[flux_dirn];

	if (fabs(fluxterm) > 1.0e-6 && nflx < 500) {
	#pragma omp critical
	{
    printf("dir=%d i=%d j=%d k=%d flux=% .8e tau_rhs=% .8e\n",
           flux_dirn,
           i,j,k,
           fluxterm,
           tau_rhs[index]);
