...
int con2prim_Palenzuela1D( const igm_eos_parameters eos,
  ...
  //--
  //static int calls=0;calls++;if(calls<=20) {printf("PALENZUELA ENTER %d\n",calls);printf("evolve_T=%d which_routine=%d\n",(int)eos.evolve_T,stats.which_routine;fflush(stdout);}
  //--
    ...
    const double rel_eps)
{
    ...
    // This is the clearest sign that energy inversion failed physically.
    // T_min is the minimum tabulated temperature.
    const bool energy_at_floor =
        (temp_energy <= 1.01 * eos.T_min);

    // --- Criterion 2: EOS is weakly temperature-sensitive ---
    // When depsdT is below the threshold, the energy inversion
    // cannot reliably distinguish temperatures. This is what
    // check_depsdT_condition also uses for the solver selection.
    const bool weak_depsdT =
        (depsdT < eos.depsdT_threshold);

    // --- Criterion 3: the two branches strongly disagree in T or eps ---
    // If they disagree by more than 10% in temperature or eps,
    // the energy branch is likely not trustworthy.
    const bool large_T_discrepancy   = (rel_T   > 0.1);
    const bool large_eps_discrepancy = (rel_eps  > 0.1);

    // --- Criterion 4: low density / surface / atmosphere region ---
    // rho_atm is 1e-11 from your LS220_rns.par.
    // We switch in a band just above the atmosphere.
    // You can tune rho_switch later.
    const double rho_switch = 1e-6;  // tune this
    const bool low_density = (rho < rho_switch);

    // Final decision:
    // Switch to entropy if in the low-density/surface region
    // AND the energy branch shows at least one sign of failure.
    ...
    const bool energy_unreliable =
      ...
      weak_depsdT;   // independent trigger
    
    const bool switch_to_entropy = low_density && energy_unreliable;
    ...
    output_stats& stats)
{
    ...
    // depsdT is not available here directly, so we use rel_T and rel_eps
    // as the reliability indicators. Pass 0.0 for depsdT to force the
    // discrepancy criteria to drive the decision.
    const bool entropy_wins = use_entropy_final(
						eos,
						rho,
						temp_energy,
						depsdT_local,   // now real
						rel_T,
						rel_P,
						rel_eps);
    ...
    // Dedicated hot-cell trap: independent of diag_count, triggers on temperature only
    // Temporary: catch ANY departure from cold, regardless of density
    if (temp_energy > 0.01) {
      fprintf(stderr,
	      "HOT2: rho=%e W=%e T_E=%e T_S=%e sel=%s\n",
	      rho, W, temp_energy, temp_entropy,
	      entropy_wins ? "S" : "E");
    }
    ...
    if (diag_count < 2000 && rho > 1e-10) {
      fprintf(stderr,
            "DE: rho=%e T_E=%e T_S=%e sel=%s floor=%d bigT=%d rswitch=%d\n",
            rho,
            temp_energy,
            temp_entropy,
            entropy_wins ? "S" : "E",
